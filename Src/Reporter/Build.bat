@rem ---------------------------------------------------------------------------
@rem Script used to build the DelphiDabbler Version Information Spy File
@rem Reporter DLL
@rem
...
echo DelphiDabbler VIS Reporter DLL Build Script
echo -------------------------------------------
...
set HTMLBase=HTML
set HTMLSrc=%SrcDir%%HTMLBase%.rc
set HTMLRes=%BinDir%%HTMLBase%.res
echo Compiling %HTMLSrc% to %HTMLRes%
%BRCC32Exe% %HTMLSrc% -fo%HTMLRes%
if errorlevel 1 set ErrorMsg=Failed to compile %HTMLRes%
if not "%ErrorMsg%"=="" goto Error
